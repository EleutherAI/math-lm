...
def main():
    ...
    total_num_tokens
    pbar = tqdm.tqdm()
    for i, (doc, bytes_processed, num_tokens) in enumerate(encoded_docs, start=1):
        ...
        if i % args.log_interval == 0:
            ...
            pbar.set_description(
                    f"Processed {i}{'' if args.num_docs is None else '/' + str(args.num_docs)} documents, {total_num_tokens:e} tokens, ({i / elapsed} docs/s, {mbs} MB/s)."
            )
            if i != 0:
                pbar.update(args.log_interval)
    
    print(f"Total tokens: {total_num_tokens:e}")
